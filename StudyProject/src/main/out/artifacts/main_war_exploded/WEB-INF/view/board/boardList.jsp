...
<script type="text/javascript" src="${root}/resource/js/boardlist.js"
	defer></script>
</head>
<body>
	<!-- 현재 페이지에 변수선언 -->
	...
	<div class="boardList">
		...
		<c:forEach var="bl" items="${blist}">
			<div class="row">
				...
				<div class="cell">
					<a href="/board/boardetail?brdno=${bl.brdno}&page=${page}&field=${field}&keyword=${keyword}">${bl.brdtitle}</a>
				</div>
		...
		<div class="status-write">
			<c:if test="${!empty sessionScope.userMap }">
				<div class="status-regedit">
					<button type="button" onclick="/board/regedit" id="regedit">글쓰기</button>
				</div>
				<!-- MyReadList -->
				<div class="myread-header">
					<button type="button" id="myreadlist">MyReadList</button>
				</div>
			</c:if>
		</div>
	</div>
